...
if($jml_baris>0){
	$admin .="<table border=1><th colspan=4 align='center'>Daftar Member Rumah Pintar</th>
<tr><td align='center' width=100px>Username</td><td align='center' width=100px>Password</td><td align='center' width=100px>Email</td><td align='center' width=100px>Hapus?</td></tr>";

while($data = mysqli_fetch_array($hasil,MYSQLI_NUM)){
	$admin .="<tr><td>$data[0]</td><td align='right'>$data[3]</td><td align='right'>$data[1]</td><td align='center'><a href=index.php?proses=del&username=$data[0]>hapus</a></td></tr>";
}
$admin .="</table>";
}
...
{$admin = "<p>Username atau password salah.<br>$kembali</p>";
}else
{
...
$username = filter_str($_GET['username']);
// echo $username;
$hasil=mysqli_query($koneksi,"DELETE FROM `data_user` WHERE `data_user`.`username` = '$username'");
if(!hasil)
	$admin = "<p>Data gagal dari sistem. <a href='index.php'>kembali</a></p>"; else
	$admin = "<p>Data telah dihapus dari sistem. <a href='index.php'>kembali</a></p>";
...
$hasil = mysqli_query($koneksi,"INSERT INTO `admin` (`username`, `email`, `password`) VALUES ('$username', '', '$password')");
if (!$hasil)
$admin = "Error: Gagal memasukkan ke database. Mungkin account "
."sudah dimasukkan. <br>$kembali";
else
